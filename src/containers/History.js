        ...
        return(
            <div>
                ...
                <ul className='list-group'>
                    {this.props.history.map((history, index) => (<li key={index} className='list-group-item list-group-item-action' onClick={() => {this.props.selectHistory(index) }}>{`${history} = ${eval(history)}`}</li>))}
                </ul>
...
function mapDispatchToProps (dispatch){
    return bindActionCreatros({ selectHistory, clearHistory}, dispatch)
}
